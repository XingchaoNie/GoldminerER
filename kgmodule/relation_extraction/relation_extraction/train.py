...
def train(hparams):
    ...
    log_dict = {'Training/training loss': [], 'Validation/f1': [], 'Validation/precision': [], 'Validation/recall': [], 'Validation/accuracy': []}
    ...
    for epoch in range(epoch_offset, epochs):
        ...
        for i_batch, sample_batched in enumerate(tqdm(train_loader, desc='Training')):
            ...
            if i_batch % 10 == 9:
                ...
                log_dict['Training/training loss'].append(running_loss / 10)
                running_loss = 0.0
        ...
        if validation_file:
            ...
            with torch.no_grad():
                ...
                log_dict['Validation/f1'].append(f1)
                # writer.add_scalar('Validation/precision', precision, epoch)
                log_dict['Validation/precision'].append(precision)
                # writer.add_scalar('Validation/recall', recall, epoch)
                log_dict['Validation/recall'].append(recall)
                # writer.add_scalar('Validation/accuracy', accuracy, epoch)
                log_dict['Validation/accuracy'].append(accuracy)
    ...
    # writer.close()
    pickle.dump(log_dict, open('./saved_models/' + time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime()) + '.pkl', 'wb'))
