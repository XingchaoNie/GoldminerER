...
def predict(hparams, sens, h_node, t_node):
    """
    ...
    # TODO: 这里改成关系类型总数
    logits = torch.zeros(relation_cnt)
    if match_obj1 and match_obj2:  # 姑且使用第一个匹配的实体的位置
        ...
        print('实体', h_node, '和', t_node, "间最大可能的关系是：{}".format(idx2tag[logits.argmax(0).item()]))
    ...
    return idx2tag[logits.argmax(0).item()] if logits.argmax(0).item() > 0.5 else ""
