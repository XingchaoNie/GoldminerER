...
class MyTokenizer(object):
    ...
    def tokenize(self, item):
        ...
        sent0 = self.bert_tokenizer.tokenize(sentence[:pos_min[0]])  # 第一个实体之前的句子
        ent0 = self.bert_tokenizer.tokenize(sentence[pos_min[0]:pos_min[1]])   # 第一个实体
        sent1 = self.bert_tokenizer.tokenize(sentence[pos_min[1]:pos_max[0]])   # 第一个实体与第二个实体中间的句子
        ent1 = self.bert_tokenizer.tokenize(sentence[pos_max[0]:pos_max[1]])   # 第二个实体
...
class SentenceREDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        encoded = self.tokenizer.bert_tokenizer.encode_plus(sample_tokens, max_length=self.max_len, pad_to_max_length=True, truncation=True)
        # 这个truncation=True是后来加的
