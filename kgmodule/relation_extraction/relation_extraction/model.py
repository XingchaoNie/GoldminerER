...
class SentenceRE(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        m_path = os.path.join(father(father(os.path.abspath(__file__))), r'pretrained_models', r'bert-base-chinese')
        self.pretrained_model_path = m_path
        # self.pretrained_model_path = hparams.pretrained_model_path
        self.embedding_dim = hparams.embedding_dim
        self.dropout = hparams.dropout
        self.tagset_size = hparams.tagset_size

        self.bert_model = BertModel.from_pretrained(self.pretrained_model_path, local_files_only=True)
    ...
    def forward(self, token_ids, token_type_ids, attention_mask, e1_mask, e2_mask):
        ...
        # 每个实体的所有token向量的平均值
        # sequence_output: 句子经过bert编码后的向量
        # e1_h和e2_h分别是实体1和实体2的向量表示
