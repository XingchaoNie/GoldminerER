...
import torch
import os
from power.entity_extraction.BertNER_Chinese.utils import batch_variable
...
def predict(config, model, one_iter, output_dict=True):
    ...
    # }
    predict_all = {
        'RCLSB': [], 'RCLSBName': [], 'RCLSBNo': [], 'RCLSBCate': [], 'RCLSBType': [], 'RCLSBJrfs': [],
        'RCLGZ': [], 'RCLGZNo': [], 'RCLGZName': [], 'RCLGZDWG': [],
        'TL': [], 'TLNo': [], 'TLName': [],
        'LQJZ': [], 'LQJZNo': [], 'LQJZName': [],
        'RCLCS': [], 'RCLCSBfjg': [], 'RCLCSBffs': [], 'RCLCSZll': [], 'RCLCSYxhd': [], 'RCLCSBwwd': [], 'RCLCSSwsj': [], 'RCLCSBwsj': [], 'RCLCSZysj': [], 'RCLCSLqjz': [], 'RCLCSLqsj': [], 'RCLCSJzwdbhfw': []
    }
    with torch.no_grad():
        # TODO: 张博确认好实体关系类型之后在这里改
        ...
        pre_path = os.path.join(father(os.path.abspath(__file__)), r'data', r'dict')
        # print(pre_path)
        # TODO:张博确认好实体关系类型之后在这里改
